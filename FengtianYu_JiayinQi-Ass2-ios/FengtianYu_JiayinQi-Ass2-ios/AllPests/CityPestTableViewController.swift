//
//  CityPestTableViewController.swift
...
// this class is for search a pest according to the city
class CityPestTableViewController: UITableViewController ,DatabaseListener, UISearchBarDelegate{
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        var firstUpperText = text.capitalizingFirstLetter()
        if firstUpperText.count > 0 {
            filteredPests = allPest.filter({ (pest: Pest) -> Bool in
                return pest.cities.contains(firstUpperText) ?? false
            })
        } else {
            filteredPests = allPest
        }
    ...
    override func tableView(_ tableView: UITableView,didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "CityPestToDetail", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
       if segue.identifier == "CityPestToDetail" {
